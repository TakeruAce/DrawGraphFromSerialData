...
color colorArray[] = {#058ED9, #483D3F,#E3B44D, #A39A92, #23CE6B, #77685D, #BA6E6E, #DF2935, #DA7422};
...
float sliderVal2;
float sliderVal3;
float sliderInitialValues[] = {0.01,0.000,0,0};
float sliderRange[] = {1.0,0.05,0.5};
...
ArrayList<ArrayList<Float>> serialEvent()
{
    ...
    if (message != null) {
      ...
      if (!CORRECT_DATA_NUM_FLAG || list[0].equals(header) && list.length == CORRECT_DATA_NUM + 1) {
        for (int i_input=0; i_input < min(list.length - 1,GRAPH_DATA_NUM) ;i_input++) {
          ...
          input_data.add(float(list[i_input+1]));
        }
      }
    }
...
class graphMonitor {
    ...
    void graphDrawForArray(ArrayList<ArrayList<Float>> new_data) {
      //New Dataを追加
    ...
    void setTimeRange(int new_time_range) {
...
void generateDummy() {
  ...
  for (int i_data = 0;i_data<GRAPH_DATA_NUM;i_data++) {
    dummy.add(cp5.getController("ampslider"+i_data).getValue() * sin(2*PI*0.5*(millis()/1000.0 - (1.0 / GRAPH_DATA_NUM * i_data) * 2.0)));
  }
  data.add(dummy);
}

void graphDataNumReset() {
